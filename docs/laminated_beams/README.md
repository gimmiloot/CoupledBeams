...
[Подключение пружины к RLB и проверка RLB → EB](inplane_rotational_spring_rlb_eb_limit.md):
один четырёхслойный изотропный контроль, матричный предел при фиксированной
kappa=1, шесть новых спектральных групп. 36 целевых частот приняты;
один новый guard квалифицирован, старые EB-неопределённости сохранены.
[Шесть угловых EB-карт при пяти состояниях узла](inplane_rotational_spring_eb_beta_maps.md):
994/1005 BASE-точек подтверждены, пропуски показаны разрывами;
общая таблица, шесть PNG/PDF и отдельный plot-only, без нового расчёта RLB.
[Продолжение шести EB-мод по формам, kappa=1 и 100](inplane_rotational_spring_eb_tracked.md):
пять выбранных пропусков закрыты в новом наборе; 402 BASE и 58 локальных
точек, массовый MAC, два графика ветвей. Шесть пересечений поддержаны
точными классами симметрии и сменой знака частотной разности.
[Точный шарнир и сравнение соответствующих EB-ветвей kappa=0,1,100](inplane_rotational_spring_eb_hinge_comparison.md):
201 точка шарнира из сохранённых корней, независимые кратные формы при 90°,
проверенное соответствие seed, шесть сравнений и один обзор. Старые формы
и события kappa=1,100 сохранены; оформление меняется через plot-only.
[Проверка механизма пружины в H/L/L/H EB/RLB при небольшом различии длин](inplane_spring_robustness.md):
72 контрольные точки и два локальных типа проверки. Симметричная независимость
семейства сохраняется; при mu=.01 выявлены небольшая зависимость вдали от
смешения, смещение первой слабой области и разрешённое veering выбранной пары.
Четыре PNG/PDF; неоднозначные высокие назначения и guard квалифицированы отдельно.
[Первый пилот Кельвина—Фойгта](inplane_kelvin_voigt_pilot.md): импортирована
теория, проверены комплексные матрицы EB/RLB, четыре упругих seed и 12 состояний
двух выбранных мод при положительной вязкости. Частоты, затухание, энергетические
контроли и missing-only сохранены; полный комплексный спектр не исследовался.
[Внешние benchmark Failla/Hong](inplane_kelvin_voigt_literature_benchmarks.md):
15 корней прошли внутренние проверки, но часть печатных разрядов не
