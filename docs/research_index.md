...
| Anisotropic rods | `planned` | Define a physically meaningful constitutive and orientation model while preserving the isotropic limit | [preparation notes](refactoring/pre_refactor_inventory/anisotropic_preparation_notes.md) | none | Constitutive scope is not fixed and no code exists. A separate assumptions note, isotropic-limit contract, and independent verification are required before implementation. |
