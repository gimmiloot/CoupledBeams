...
class CacheAndIntegrationTests(unittest.TestCase):
    def test_stale_cache_version_is_rejected_and_new_cache_reused(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            cache.save(result)
            payload = json.loads(path.read_text(encoding="utf-8"))
            payload.pop("eb_matrix_evaluator_version")
            path.write_text(json.dumps(payload), encoding="utf-8")
            self.assertIsNone(cache.load(complete.MODEL_EB, geometry, settings))
            self.assertEqual(cache.last_load_status, "stale_cache_eb_matrix_evaluator")
            cache.save(result)
            payload = json.loads(path.read_text(encoding="utf-8"))
            payload.pop("timoshenko_basis_evaluator_version")
            path.write_text(json.dumps(payload), encoding="utf-8")
            self.assertIsNone(cache.load(complete.MODEL_EB, geometry, settings))
            self.assertEqual(cache.last_load_status, "stale_cache_timoshenko_basis_evaluator")
            cache.save(result)
