...
| Reddy laminated-beam stiffness-layout contrast map | `analysis/laminated_beams/sweep_reddy_stiffness_layout_contrast.py` | `completed` finite sorted-spectrum map — `PASS` | [RLB-2E note](../docs/laminated_beams/reddy_stiffness_layout_contrast_sweep.md) | Use only for the declared fixed geometry and `chi` grid. Positions 1--8 are sorted positions, root 9 is guard-only, and `--plot-only` is the presentation path. Do not infer branch identity, veering, shapes, or localization. |
| Reddy one-arm layered contrast map | `analysis/laminated_beams/sweep_reddy_one_arm_layered_contrast.py` | `completed` finite sorted-spectrum map — `PASS` | [RLB-2F note](../docs/laminated_beams/reddy_one_arm_layered_contrast_sweep.md) | The declared signed `xi` contract uses one layered arm and one homogeneous reference arm. All 81 points and root-9 guards are complete; 13 neighbour flags were reproduced by local repair. Positions 1--8 are sorted, root 9 is guard-only, and no branch, shape, crossing, veering, or localization claim is supported. |
| Reddy mass-layout duality maps | `analysis/laminated_beams/sweep_reddy_mass_layout_duality.py` | `completed` finite sorted-spectrum maps — `PASS` | [RLB-2G note](../docs/laminated_beams/reddy_mass_layout_duality.md) | The declared `eta` and signed `xi_rho` grids preserve elastic stiffnesses and mass per length; only rotary inertia `J` changes. All 204 logical groups and root-9 guards are complete, with no neighbour flag or unresolved point. Positions 1--8 are sorted, root 9 is guard-only, and no general mass law, branch, shape, crossing, veering, or localization claim is supported. |
| Reddy axial-stiffness visibility maps | `analysis/laminated_beams/sweep_reddy_axial_stiffness_visibility.py` | `completed` finite sorted-spectrum maps — `PASS` | [RLB-2H note](../docs/laminated_beams/reddy_axial_stiffness_visibility.md) | The declared A-only grid contains 62 complete groups at `beta=0,30 deg`; 14 neighbour flags were reproduced and no point is unresolved. The beta=0 direct matrices only check singularity at frequencies supplied by the reused coupled/exact-subsystem union; they do not localize frequencies independently. Positions 1--8 are sorted and root 9 is guard-only, so no branch, shape, crossing, veering, or localization claim is supported. |
| Reddy six-ply global-equivalence diagnostic | `analysis/laminated_beams/analyze_reddy_six_ply_equivalent_laminates.py` | `completed` finite layerwise diagnostic — `PASS` | [RLB-2I note](../docs/laminated_beams/reddy_six_ply_equivalent_laminates.md) | The exact 51-point constitutive family preserves `A,D,S,m,J` while redistributing ply stresses and energy. Spectral work is limited to six roots-1--9 regressions; no wide frequency map, strength criterion, branch tracking, mode shapes, Ritz, or FEM is included. |
| Reddy six-ply pairwise stiffness-transfer maps | `analysis/laminated_beams/sweep_reddy_six_ply_pairwise_stiffness_transfer.py` | `completed` finite sorted-spectrum maps — `PASS` | [RLB-2J note](../docs/laminated_beams/reddy_six_ply_pairwise_stiffness_transfer.md) | Three 81-point transfer maps keep `A,S,m,J` fixed while `D` changes with levers 1:2:3. All 25 neighbour flags were reproduced with no unresolved point, and exact matched-`D` spectra collapse. Positions 1--8 are sorted, root 9 is guard-only; no branch, mode, crossing, veering, or localization claim is supported. |
| Isotropic-circular epsilon article assets v1 | `analysis/thickness_mismatch/article/build_article_epsilon_upper_envelope_assets.py` | `completed` zero-solve article postprocessing | `results/article_epsilon_upper_envelope/article_ready_v1/` | Reads only the final compact result layer, includes eight complete epsilon levels in the main finite-grid envelope, and keeps S3_14/S3_12 as separate regression controls. |
| Isotropic-circular longitudinal-energy triplet pilot | `analysis/thickness_mismatch/audits/audit_article_longitudinal_energy_examples.py` | `completed` article-facing pilot | `results/article_epsilon_upper_envelope/energy_triplet_pilot_v1/` | Frequency-only selection fixes two geometries before energy reconstruction; exactly six same-index sorted Timoshenko forms are checked on 801/1601 grids. No branch tracking, 70% classification, shape plotting, root solve, strict verification, MAC, FEM, or anisotropic workflow is used. The two examples are illustrative, not statistical. |
| Isotropic-circular coupled-angle longitudinal-energy pilot | `analysis/thickness_mismatch/audits/audit_article_longitudinal_energy_examples.py --coupled-angle-pilot` | `completed` article-facing pilot | `results/article_epsilon_upper_envelope/energy_triplet_coupled_pilot_v2/` | Frequency-only selection fixes two `beta=15 deg` geometries before reconstructing exactly six same-index sorted Timoshenko forms. The first satisfies `s_max<=0.1`; the second is explicitly in the extended one-dimensional-model range. The immutable `beta=0` v1 bundle is read only as a zero-solve control. No physical-branch tracking, formal 70% classification, root/strict/MAC/shape/FEM/anisotropic workflow, or statistical claim is used. |
| Isotropic-circular additional-angle energy extension | `analysis/thickness_mismatch/audits/audit_article_longitudinal_energy_examples.py --additional-angle-pilot` | `completed` article-facing figure pilot | `results/article_epsilon_upper_envelope/energy_triplet_angle_extension_v3/` | Case-ID-deduplicated frequency-only selection fixes one `epsilon_0=0.030`, `beta>=30 deg`, `s_max<=0.1` geometry before reading prior energy data; only its three saved sorted roots are reconstructed. Compact beta-15/30 and extended three-example PNG candidates are generated while v1/v2 remain immutable. No article section, branch/MAC/shape/root/strict/FEM/anisotropic work, or 70% classification is used. |
...
| Article epsilon finite-grid upper envelope | `analysis/thickness_mismatch/audits/run_article_epsilon_upper_envelope_grid.py`; `lib/article_epsilon_compact_certificates.py`; `lib/article_epsilon_compact_poststage.py`; `lib/article_epsilon_targeted_resolution.py` | `active-diagnostic` / primary queue completed / compact finalization / epsilon-0.050 envelope exact | [targeted report](../results/article_epsilon_upper_envelope/coarse_grid_v1/epsilon_005_targeted_resolution/report.md) | Isotropic circular rods only; separate from both the closed Rule A/B/S study and rectangular-anisotropic research. The 845-case production primary queue completed with atomic caches and no force strict, then the old parent aggregation exhausted memory by retaining full decompressed traces. Compact certificates make family processing memory-bounded. The two epsilon-0.050 blockers were selected from the unresolved table and resolved by target-only local matrix verification without force/full strict, finalizing the observed maximum at 4. Full raw caches remain retained evidence; no prune action ran. |
| Sorted-family inventory local-repair audit, shadow integration, and promotion reconciliation | `analysis/thickness_mismatch/audits/audit_family_inventory_local_repair.py`; `lib/family_inventory_local_repair.py`; `lib/article_epsilon_family_inventory_integration.py`; `lib/article_epsilon_family_reconciliation.py` | standalone/shadow/reconciliation gates `PASS` / runner default `off` / production resume unexecuted | [reconciliation report](../results/article_epsilon_upper_envelope/coarse_grid_v1/family_local_repair_reconciliation/report.md) | Sorted ranks remain the comparison basis. The explicit reconciliation CLI promotes only matrix-confirmed verified shadow rows, preserves the point-cache namespace, and leaves every failed repair deferred with `N_true=NaN`. The production queue consumes the reconciled resume plan. No detector, repair, matrix evaluator, strict path, or coarse-grid solve runs during reconciliation. |
...
| Counterexample frequency plots | `analysis/thickness_mismatch/maps/plot_counterexample_dimensional_frequency_beta.py` | `completed` | [frequency-map policy](../docs/numerics/frequency_map_computation_policy.md) | Use saved certified CSV data and `--plot-only` for presentation changes. |
