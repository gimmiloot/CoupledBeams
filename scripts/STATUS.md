...
| Article epsilon finite-grid upper envelope | `analysis/thickness_mismatch/audits/run_article_epsilon_upper_envelope_grid.py`; `analysis/thickness_mismatch/benchmarks/benchmark_article_epsilon_prefix_optimization.py`; `analysis/thickness_mismatch/benchmarks/benchmark_article_epsilon_prefix_parallelism.py`; `analysis/thickness_mismatch/audits/audit_article_epsilon_solver_readiness_v2.py`; `analysis/thickness_mismatch/postprocess/analyze_article_epsilon_upper_envelope.py` | `active-diagnostic` / equivalence `PASS` / solver readiness `PASS` / interrupted coarse grid 320/1554 / resume readiness `PASS` | [partial interruption report](../results/article_epsilon_upper_envelope/coarse_grid_v1/partial_report.md) | Separate from the closed Rule A/B/S study. Full K10 remains the reference/default. Zero-solve audit accepts 309 exact prefixes, leaves 11 genuinely prefix-affecting unresolved, one interrupted cache, and 1233 not attempted. The isolated 64-point-per-setting benchmark is deterministic for workers 1/2/4 and recommends four geometry-chain workers; this does not resume the main grid or make the partial envelope article-ready. |
| Sorted-family inventory local-repair audit | `analysis/thickness_mismatch/audits/audit_family_inventory_local_repair.py`; `lib/family_inventory_local_repair.py` | `diagnostic-only` / gates A--F `PASS` | [audit report](../results/article_epsilon_upper_envelope/family_inventory_local_repair_audit/report.md) | Sorted ranks remain the comparison basis. The detector infers local beta/Lambda repairs before deferred strict, but it is not integrated into the production runner; the coarse grid was not resumed. |
