...
| Article epsilon finite-grid upper envelope | `analysis/thickness_mismatch/audits/run_article_epsilon_upper_envelope_grid.py`; `analysis/thickness_mismatch/benchmarks/benchmark_article_epsilon_prefix_optimization.py`; `analysis/thickness_mismatch/benchmarks/benchmark_article_epsilon_prefix_parallelism.py`; `analysis/thickness_mismatch/audits/audit_article_epsilon_solver_readiness_v2.py`; `analysis/thickness_mismatch/postprocess/analyze_article_epsilon_upper_envelope.py` | `active-diagnostic` / equivalence `PASS` / solver readiness `PASS` / reconciliation gates `PASS` / production resume unexecuted | [reconciled report](../results/article_epsilon_upper_envelope/coarse_grid_v1/family_local_repair_reconciliation/report.md) | Isotropic circular rods only; separate from both the closed Rule A/B/S study and rectangular-anisotropic research. The immutable approved inventory contains 684 original accepted results; verified shadow promotion adds seven resolved results and retains 18 deferred cases without `N_true`. Of 846 manifest IDs without an approved source point cache, one benchmark is already deferred, so the deterministic future queue contains 845. Reconciliation is explicit, zero-solve, idempotent, and does not overwrite point caches or resume the grid. |
