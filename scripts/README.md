...
For thickness-mismatch fixed-eta diagnostics, the same convention applies with
the local start point used by the script: `branch k` means the descendant of the
k-th mode shape at `mu=0`. The sorted root index at later `mu` values is only
diagnostic metadata. A sorted-position jump larger than one step, for example a
reported 5-to-7 jump, must be flagged as suspicious and checked on a refined
local mu grid before it is interpreted physically.
Low-MAC or low-margin candidate assignments cannot change accepted descendant
identity or accepted canonical sorted position; scripts must mark such steps as
unresolved and retain the previous canonical sorted position until a refined
local check resolves the ambiguity.
...
## FEM right-arm transform convention

The production FEM right-arm convention is local-to-global:
`q_global = T @ q_local`, with right axial mapped to `(cos beta, sin beta)` and
right transverse mapped to `(-sin beta, cos beta)`. The right-arm matrices are
assembled as `K_global = T @ K_local @ T.T` and
...
and output notes live in `../docs/thickness_mismatch/README.md`. The
...
| Descendant eta sweep | `python scripts/analysis/plot_lambda_mu_thickness_mismatch_beta15_eta_descendants.py --betas 7.5` | Building clean analytic-only descendant plots for selected beta angles; no args reproduces the beta-15 plot. |
...
| Eta=0.5 crossing audit | `python scripts/analysis/audit_eta0p5_eps0p0025_lambda_mu_crossings.py` | Testing descendant branch crossings separately from sorted-root metadata and tracking artifacts. |
| Article beta=5 desc1/desc2 crossing audit | `python scripts/analysis/audit_article_beta5_eta0p5_desc1_desc2_crossing.py` | Checking whether the apparent article-figure crossing at `beta=5 deg`, `epsilon=0.0025`, `eta=0.5` is real or a coarse-tracking/root-extraction artifact. |
| Eta=0 mu-scan first-six rearrangement audit | `python scripts/analysis/audit_mu_scan_eta0_first6_rearrangement.py` | Checking where first-six descendants change sorted position over beta as `mu` varies at `eta=0`; diagnostic-only, not article figure generation. |
| Eta=0 fixed-mu `Lambda(beta)` rearrangement check | `python scripts/analysis/plot_lambda_beta_eta0_eps0p0025_mu_rearrangement_check.py` | Plotting tracked first-six `Lambda(beta)` descendants for `mu=0.001,0.002,0.003` with refined beta spacing and spike/tracking-artifact checks. |
...
| Branch-2 mode-shape beta transition audit | `python scripts/analysis/audit_mode_shape_branch2_beta_transition_eta0p1_mu0p3.py` | Auditing the sorted 2-3 gap and descendant 2/3 tracking around the eta=0.1, mu=0.3 branch-2 mode-shape beta scan transition; sorted gaps decide crossing status, while descendant labels remain diagnostics. |
| Eta=+-0.5 sorted `Lambda(mu)` with single-rod references | `python scripts/analysis/plot_lambda_mu_eta_m0p5_with_single_beam_refs.py --eta 0.5 --betas 0 5 10 15` | Plotting diagnostic-only sorted coupled `Lambda(mu)` curves with CP/FP and CC/FF single-rod reference families computed from `alpha*sqrt(tau_i)/L_i`; default PNGs zoom to the coupled spectrum, clip reference segments only in the plot, and style references by the actual short/thin or long/thick rod role. |
| Eta-mu-beta global trend post-processing | `python scripts/analysis/summarize_diagnostic_eta_mu_beta_global_trends.py` | Reading existing diagnostic map CSVs to summarize global mu/eta trends, eta-sign asymmetry, pair transitions, beta-at-minimum histograms, branch sensitivity dominance, and surrogate-fit smoothness. |
| Descendant-6 veering shapes | `python scripts/analysis/plot_thickness_mismatch_desc6_veering_mode_shapes.py` | Selecting three distinct sorted-gap local minima for pairs 4-5 and 5-6, then plotting descendant-6 shapes at those six eta=0.5 points. |
| Descendant-6 localization audit | `python scripts/analysis/audit_desc6_veering_localization.py` | Computing rod-wise displacement, slope, curvature, and tau-weighted EB bending-energy localization metrics for the six selected descendant-6 veering shape points. |
...
The determinant nullspace conditioning audit runs behind `--audit-determinant-nullspace-conditioning`. It compares the raw determinant SVD null vector with coefficients reconstructed from the working joint-displacement dynamic-stiffness null vector, then repeats determinant null-vector extraction under row normalization, column normalization, combined row/column normalization, and several axial-column physical scale candidates. The CSV `results/determinant_nullspace_conditioning_*.csv` records singular values, raw and row-normalized determinant residuals, labeled per-row residual components in `notes`, `q_joint` MAC/L2 against the joint-null reference, FEM residuals from the current determinant reconstruction path, and Rayleigh Lambda. This is diagnostic postprocessing only and does not replace determinant-null reconstruction with joint-null reconstruction.

The determinant-as-joint-system derivation runs behind `--derive-determinant-as-joint-system`. It forms the coefficient-to-joint map `q_joint = Q c`, restricts the determinant force rows to the kinematic nullspace `c = N a` of the first three determinant rows, inverts `Q N` when it is a regular `3x3` map, and writes the induced physical joint operator `D_det_joint = force_scale * Rf N (Q N)^(-1)` in `[Fx,Fy,M]` rows and `[ux,uy,theta]` columns. The outputs are `results/determinant_as_joint_system_..._matrix.csv`, `*_summary.csv`, and `results/determinant_as_joint_system_variants_....csv`, including current, mixed right-arm sign, transverse-row sign, axial-row sign, and swapped-Fx/Fy mapping variants. This checks algebraic equivalence of the determinant force equations and the working joint dynamic stiffness on the determinant kinematic subspace only; it does not modify `formulas.py` or production reconstruction.

### Joint displacement dynamic-stiffness nullspace comparison

- Task: compare the coupled-joint equation through the physical joint displacement vector `q_joint=[ux,uy,theta]`, avoiding determinant internal-force sign conventions.
- Command: `python scripts/analysis/compare_joint_dynamic_stiffness_analytic_fem.py --branch-id bending_desc_05 --beta 15 --mu 0.2 --epsilon 0.0025 --compare-joint-nullspace`
- Results: writes `results/joint_nullspace_compare_..._matrix.csv`, `*_summary.csv`, `*_vectors.csv`, `*_pairwise.csv`, and `results/joint_nullspace_reconstruction_residual_....csv`.
- Use when: `N`, `Q`, `M` row conventions are ambiguous and you need to compare the actual `3x3` joint displacement operators satisfying `D_total(Lambda) @ q_joint = 0`.

This mode forms `D_total_FEM = D_left_FEM + D_right_FEM_global` from the existing Schur complement path and `D_total_analytic = D_left_analytic + D_right_analytic_global` from the independent full-basis arm dynamic stiffness. It writes Frobenius/max-entry matrix differences, condition numbers, singular values, SVD null vectors, and pairwise MAC/L2 comparisons among the FEM eigenvector joint displacement, determinant-coefficient joint displacement, FEM Schur null vector, and analytic dynamic-stiffness null vector. The reconstruction residual CSV compares the current determinant-null-vector embedding with a diagnostic reconstruction solved from `q_joint_null_analytic_D`. This is diagnostic postprocessing only and does not modify the determinant, formulas, solvers, FEM baseline, physical model, or eigenfrequencies.

### Beta=0 single-rod determinant limit diagnostic

- Task: check whether the current determinant roots and determinant SVD null-vector reconstruction reduce to one straight clamped-clamped beam of total length 2 when `beta=0`.
- Command: `python scripts/analysis/check_beta0_single_rod_limit.py`
- Main parameters: `--epsilon`, `--mu-values`, `--num-bending-modes`, `--output-prefix`.
- Results: by default writes `results/beta0_single_rod_limit_roots.csv`, `results/beta0_single_rod_limit_shapes.csv`, `results/beta0_single_rod_limit_shape_samples.csv`, and `results/beta0_single_rod_limit_summary.csv`.
- Use when: determinant-null-vector residuals are suspect and you need to separate collinear single-rod behavior from angled-joint coupling/sign issues.
...
| `scripts/analysis/check_thickness_mismatch_branch_identity_eta_p0p5.py` | analysis/diagnostic | Refined eta=0.5 descendant-branch identity audit for thickness-mismatch branches 5--7 over mu=0.65..0.90. | `python scripts/analysis/check_thickness_mismatch_branch_identity_eta_p0p5.py` | keep in `scripts/analysis/` |
...
| `scripts/analysis/audit_eta0p5_eps0p0025_lambda_mu_crossings.py` | analysis/diagnostic | Pairwise eta=0.5 crossing audit for the beta=15, epsilon=0.0025 thickness-mismatch EB `Lambda(mu)` descendants, separating sign changes from sorted-root metadata and tracking artifacts. | `python scripts/analysis/audit_eta0p5_eps0p0025_lambda_mu_crossings.py` | keep in `scripts/analysis/` |
| `scripts/analysis/audit_article_beta5_eta0p5_desc1_desc2_crossing.py` | analysis/diagnostic | Focused article-figure check for beta=5, epsilon=0.0025, eta=0.5 descendant 1/2 crossing; uses a refined local mu grid and fine root scan to separate a real crossing from a coarse tracking/root-extraction artifact. | `python scripts/analysis/audit_article_beta5_eta0p5_desc1_desc2_crossing.py` | keep in `scripts/analysis/` |
| `scripts/analysis/audit_mu_scan_eta0_first6_rearrangement.py` | analysis/diagnostic | Analytic-only eta=0 mu scan for first-six beta-driven sorted-position rearrangement; solves first 14 sorted roots, tracks eight descendants, refines class boundaries, and writes CSV/Markdown/PNG outputs under `results/`. | `python scripts/analysis/audit_mu_scan_eta0_first6_rearrangement.py` | keep in `scripts/analysis/` |
| `scripts/analysis/plot_lambda_beta_eta0_eps0p0025_mu_rearrangement_check.py` | analysis/diagnostic | Targeted fixed-mu `Lambda(beta)` plots for `mu=0.001,0.002,0.003` at eta=0 and epsilon=0.0025, using the mu-scan MAC tracking path with beta step 0.1 and spike-artifact checks. | `python scripts/analysis/plot_lambda_beta_eta0_eps0p0025_mu_rearrangement_check.py` | keep in `scripts/analysis/` |
| `scripts/analysis/audit_eta0_mu_positive_gap_rearrangement_verification.py` | analysis/diagnostic | Strict eta=0 positive-mu adjacent-gap audit for first-six/seven sorted roots, with beta-local gap minimization, default-vs-strict root comparisons, determinant residual diagnostics, and separate descendant-label tracking notes. | `python scripts/analysis/audit_eta0_mu_positive_gap_rearrangement_verification.py` | keep in `scripts/analysis/` |
| `scripts/analysis/plot_diagnostic_eta_mu_beta_frequency_maps.py` | analysis/diagnostic | Diagnostic-only sorted-frequency maps for the thickness-mismatch EB determinant: `Lambda(eta)` at beta=0/mu=0, requested sorted `Lambda(beta)` slices, and eta-mu heatmaps of minimum adjacent gaps and beta-sensitivity metrics. | `python scripts/analysis/plot_diagnostic_eta_mu_beta_frequency_maps.py` | keep in `scripts/analysis/` |
| `scripts/analysis/plot_mode_shapes_eta_beta_scan.py` | analysis/diagnostic | Diagnostic-only analytic Euler-Bernoulli thickness-mismatch mode-shape beta scan for one fixed-mu eta-aware descendant branch, writing per-beta PNGs, a contact-sheet PNG, and a branch/sorted-index summary CSV. | `python scripts/analysis/plot_mode_shapes_eta_beta_scan.py` | keep in `scripts/analysis/` |
| `scripts/analysis/plot_mode_shapes_eta_beta_scan_sorted_modes.py` | analysis/diagnostic | Diagnostic-only analytic Euler-Bernoulli thickness-mismatch mode-shape beta scan for fixed sorted modes 2 and 3, writing per-beta PNGs, per-mode and comparison grids, a summary CSV, and a sorted 2-3 gap CSV/PNG; no descendant tracking is used for selection, and `--sign-normalization short-rod-up` is the default plotting sign convention. | `python scripts/analysis/plot_mode_shapes_eta_beta_scan_sorted_modes.py` | keep in `scripts/analysis/` |
| `scripts/analysis/audit_mode_shape_branch2_beta_transition_eta0p1_mu0p3.py` | analysis/diagnostic | Local sorted-gap and descendant-label audit for the eta=0.1, mu=0.3 descendant-2 mode-shape beta scan; writes refined sorted 2-3 gap plots, descendant 2/3 tracking map, MAC plot, CSV, and Markdown report. | `python scripts/analysis/audit_mode_shape_branch2_beta_transition_eta0p1_mu0p3.py` | keep in `scripts/analysis/` |
| `scripts/analysis/plot_lambda_mu_eta_m0p5_with_single_beam_refs.py` | analysis/diagnostic | Diagnostic-only sorted `Lambda(mu)` plots for the eta thickness-mismatch EB determinant, adding non-horizontal CP/FP and CC/FF single-rod references using `alpha*sqrt(tau_i)/L_i` and styling references by the actual short/thin or long/thick rod role; default plots use `--zoom-coupled` and `--clip-ref-curves`, with optional `--full-scale-refs`. | `python scripts/analysis/plot_lambda_mu_eta_m0p5_with_single_beam_refs.py` | keep in `scripts/analysis/` |
| `scripts/analysis/summarize_diagnostic_eta_mu_beta_global_trends.py` | analysis/diagnostic | CSV-only post-processing of the diagnostic eta-mu-beta map into global trend summaries, pair-transition summaries, beta-at-minimum histograms, branch-dominance counts, surrogate fits, report, and diagnostic plots. | `python scripts/analysis/summarize_diagnostic_eta_mu_beta_global_trends.py` | keep in `scripts/analysis/` |
...
| `scripts/analysis/plot_lambda_mu_thickness_mismatch_beta15_eta_descendants.py` | analysis/diagnostic | Plot clean presentation-style EB thickness-mismatch descendant `Lambda(mu)` branches for one or more beta angles, with three eta panels, first six displayed descendants, shape-MAC branch tracking, and solid/dashed segments for the `2*r_i/l_i <= 0.1` criterion. | `python scripts/analysis/plot_lambda_mu_thickness_mismatch_beta15_eta_descendants.py --betas 7.5` | keep in `scripts/analysis/`; no args preserves beta=15 output |
...
| `scripts/analysis/plot_thickness_mismatch_branch_shapes_vs_eta.py` | analysis/diagnostic | Plot full deformed thickness-mismatch shapes for one tracked descendant branch, overlaying eta values at target mu values and optionally writing compact output folders plus aggregate CSV/Markdown summaries. | `python scripts/analysis/plot_thickness_mismatch_branch_shapes_vs_eta.py --branch-index 5 --mus 0 0.1 0.2 --etas -0.5 0 0.5` | keep in `scripts/analysis/` |
