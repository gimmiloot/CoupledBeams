...
BETA_DEG = 15.0
BRANCH_ID = "bending_desc_04"
TARGET_MUS = (0.0, 0.1, 0.2)
MU_TRACK_VALUES = analysis_mu_grid(start=0.0, stop=max(TARGET_MUS), anchor_points=TARGET_MUS)
OUTPUT_PATH = RESULTS_DIR / "flat_mu_bending_shapes_beta15_bending_desc_04_mu0_0p1_0p2_ru.png"
...
def build_clean_shape_plot() -> Path:
    ...
    for item in normalized_cases:
        ...
        ax.set_title(
            ...
            f"Λ = {lambda_value:.3f}, место в спектре: {spectral_index}",
            fontsize=9.6,
        )
    ...
    fig.suptitle("Формы колебаний: β = 15°, потомок 4-й изгибной ветви", y=0.992, fontsize=15)
