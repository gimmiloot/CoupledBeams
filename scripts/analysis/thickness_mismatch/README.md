...
`(beta, mu, eta)=(0,0,0)`, independently verifies resolved brackets with cache
disabled and a larger EB candidate margin, writes nine audit CSVs, one Markdown
report, and six compact diagnostic plots, and supports `--smoke` and
`--plot-only`. Prefix 1 remains safe through the upper scan endpoint; prefixes
2--10 have verified first-loss brackets. The separate `mu=0.7/0.9` audit keeps
high-mode general-scan discrepancies visible and does not use them to construct
the baseline thresholds. This workflow is not a lower-envelope proof and does
not implement the step-3 geometry search.
...
| Straight-system epsilon baseline thresholds | `python scripts/analysis/thickness_mismatch/audits/audit_eb_epsilon_baseline_thresholds.py --epsilon-min 0.005 --epsilon-max 0.060 --primary-epsilon-min 0.010 --primary-epsilon-max 0.050 --coarse-step 0.0005 --k-max 10 --n-spectrum-roots 12 --n-candidate-roots 20 --epsilon-tolerance 1e-6 --delta-tolerance 1e-6 --output-dir results/eb_epsilon_baseline_thresholds --reuse-cache` | reuses the fixed-epsilon root cache, existing general EB/Timoshenko root wrappers, existing fixed-fixed EB roots, and existing local sign/SVD recovery helpers | nine `baseline_*.csv` audit tables, `eb_epsilon_baseline_thresholds_report.md`, six diagnostic PNGs, and root cache files under `results/eb_epsilon_baseline_thresholds/` | Research-step-2 baseline only at `(beta,mu,eta)=(0,0,0)`. Computes squared-Lambda dimensional-frequency errors, running prefix maxima, conservative first-loss brackets, re-entry/order/multiplicity audits, analytic straight-rod checks, separate mu-invariance diagnostics, and force-recompute threshold verification. It does not change formulas/solvers, run FEM, prove a geometry lower envelope, or implement step 3. |
